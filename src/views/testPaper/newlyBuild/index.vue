<template>
  <div class="box_r">
    <div class="wpr">
      <div class="top">
        <a href="#/testPaper"><img src="@/icons/svg/fanhui.svg" alt=""> 返回</a>
      </div>
      <el-form ref="form" :model="form" label-width="100px">
        ...
        <el-form-item label="难度系数:">
          <el-rate v-model="diffcultLevel" class="xinx" />
        </el-form-item>
      ...
      <div class="foter">
        <a @click="shengcheng">
          <el-button size="mini" type="primary">生成试卷</el-button>
        </a>
        <el-button size="mini" @click="tijiao">取消</el-button>
      </div>
    </div>
    <el-dialog
      title="新建组卷"
      ...
      width="70%"
    >
      <detailsOfExaminationPapers v-if="zd" :timu-obj="timuObj" @tijiao="tijiao" />
    </el-dialog>
...
<script>
import detailsOfExaminationPapers from '@/views/testPaper/newlyBuild/detailsOfExaminationPapers'
  ...
  watch: {
    ...
    subjectId() {
      ...
      this.kemuList.forEach(data => {
        if (this.subjectId == data.subjectId) {
  ...
  methods: {
    ...
    queren(val) {
      questionlistByKeyword({
        diffcultLevel: this.diffcultLevel,
        pointId: this.pointId.join(','),
